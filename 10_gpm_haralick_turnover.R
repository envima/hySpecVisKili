...
setwd("/media/sd19006/data/users/iotte/R-Server/hyperspectral/clean/gpm_re_norm_pci_hara_species_betadiv")
...
filepath_gpm = "/media/sd19006/data/users/iotte/R-Server/hyperspectral/clean/gpm_re_norm_pci_hara_species_betadiv/"
mrg_tbl = read.table(paste0(filepath_gpm, "rs_veg_hara_species_turnover_TO_NE_AC_df.csv"), 
                     header = TRUE, sep = ";", dec = ",")

test100 <- colSums(mrg_tbl[,c(2:1687)]) #1111 #1687 #1095 #1802 #1143 #16191
test100_4 <- which(is.na(test100))
test101 <- as.numeric(test100_4)
mrg_tbl_no = seq(2:1687) #1802 #1691
...
mrg_tbl_gpm_list = lapply(responses, function(rsp){
  ...
  filepath = paste0(filepath_gpm, "mrg_tbl_gpm_model_pci_hara_species_betadiv_", rsp, ".rds")
