...
plotVarImpHeatmapAlpha <- function (var_imp_scale, xlab = "Variable", ylab = "Method", vis_range = "minmax") 
{
  ...
  levels(temp$RESPONSE)[levels(temp$RESPONSE) == "bb"] <- "predator flying"
  #levels(temp$trophLev)[levels(temp$trophLev) == 6] <- "plants"
  
  temp = temp %>%
    group_by(VARIABLE,RESPONSE) %>% #trophLev
    summarise(avg = mean(mean)) %>%
    arrange(avg)
  colnames(temp) = c("VARIABLE", "RESPONSE", "mean") #trophLev
  ...
  lattice::levelplot(mean ~ RESPONSE * VARIABLE, data = temp, 
                     ...
                     main = "Variable Importance > 0.75 Alpha Diversity Taxa Level", 
...
var_imp_scale_gen = subset(var_imp_scale,
                           var_imp_scale$RESPONSE == "SRmammals"|
                             var_imp_scale$RESPONSE == "SRanimals"|
                             var_imp_scale$RESPONSE == "SRsyrphids"|
                             var_imp_scale$RESPONSE == "SRants"|
                             var_imp_scale$RESPONSE == "SSRsnails")
