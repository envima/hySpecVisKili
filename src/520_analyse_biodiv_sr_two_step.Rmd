...
comp2StepPred(comb_sr_two_step, model, model_res)
...
models_res$mptype = factor(models_res$mptype, levels = c("rf_gam_elev_res"))


ggplot(data = models_res[models_res$ptype == "gam_elev_res" | models_res$ptype == "spec",], aes(x = resp, y = RMSE_normSD, fill = mptype)) + 
  geom_boxplot() +
  theme_bw() + 
